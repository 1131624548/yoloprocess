...
# bbox_img为待粘贴的小图尺寸，
def random_add_patches(img_name, bbox_img, rescale_boxes, shape, paste_number, iou_thresh):
    temp = []
    for rescale_bbox in rescale_boxes:
        temp.append(rescale_bbox)
    bbox_h, bbox_w, bbox_c = bbox_img       # 待粘贴的小图尺寸
    img_h, img_w, img_c = shape     # 原图像尺寸
    center_search_space = sampling_new_bbox_center_point(shape, bbox_img)  # 选取生成随机点区域
    success_num = 0
    new_bboxes = []
    # 粘贴图像的类别，第一位粘贴图像的类别也跟上，不然后面不好生成xml
    cl = 0
